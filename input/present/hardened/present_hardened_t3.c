...
Hardening technique: cipher decryption check (T3)
Details:
  - the cipher is decrypted and compared with a given plaintext before
    being output.
*/
...
void error() {
	fprintf(stderr, "HARDERR T3 The algorithm failed to execute properly\n");
  exit(-1);
}
...
int decryption(volatile uint8_t *state_check, uint8_t *key_check) {
	const uint8_t sBox4[] = {0xc,0x5,0x6,0xb,0x9,0x0,0xa,0xd,0x3,0xe,0xf,0x8,0x4,0x7,0x1,0x2};
  ...
  //	key precomputation
    ...
    while(i<9) {
			key_check[i] = key_check[i] >> 3 | key_check[i+1] << 5;
			i++;
		}

		key_check[9] = key_check[9] >> 3 | save1 << 5;
		key_check[9] = sBox4[key_check[9]>>4]<<4 | (key_check[9] & 0xF);
		if((round+1) % 2 == 1)
			key_check[1] ^= 128;
		key_check[2] = ((((round+1)>>1) ^ (key_check[2] & 15)) | (key_check[2] & 240));

		for(i=2;i<=9;i++) {
			subkey[round+1][i-2] = key_check[i];
		}
  ...
  //	****************** End Key Scheduling ******************
    ...
    //	****************** addRoundkey *************************
		i=0;
		while(i<=7)	{
			state_check[i] = state_check[i] ^ subkey[round][i];
			temp_pLayer[i] = 0;
			i++;
		}
    //	****************** End addRoundkey *********************
    //	****************** pLayer ******************************
		for(i=0;i<64;i++) {
			...
			element_destination = position / 8;
			bit_destination = position % 8;
			temp_pLayer[element_destination] |= ((state_check[element_source]>>bit_source) & 0x1) << bit_destination;
		}

		for(i=0;i<=7;i++)	{
			state_check[i] = temp_pLayer[i];
		}
    //	****************** End pLayer **************************
    //	****************** sBox ********************************
		i=0;
    ...
    while(i<=7)	{
			state_check[i] = invsBox4[state_check[i]>>4]<<4 | invsBox4[state_check[i] & 0xF];
			i++;
		}
  ...
  //	****************** addRoundkey *************************
	i = 0;
	while(i<=7) {				//final key XOR
		state_check[i] = state_check[i] ^ subkey[0][i];
		i++;
	}
...
int main(int argc, char *argv[]){
  ...
  uint8_t key_check[10];  // duplicate key needed for decryption
	...
	for (k = 0; k < 10; k++) {
	    ...
	    sscanf(&argv[1][k * 2], "%2hhx", &key_check[k]);
  }
  ...
  // storage for a decrypted cipher (if successful should be equal to plaintext)
	volatile uint8_t state_check[] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};

	encryption(plaintext, state, key);

  // Copy cipher to state_check for verification
  for (int p=0;p<8;p++) {
    state_check[p] = state[p];
  }

  decryption(state_check,key_check);

  // compare decrypted cipher and a plaintext
  while (i<8) {
    if (plaintext[i]!=state_check[i]) error();
    i++;
  }
    ...
    if (plaintext[i]!=state_check[i]) error();  // one more check
