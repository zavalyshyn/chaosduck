...
Hardening technique: loop hardening (T0)
Details:
  - all loop counters are duplicated and verified on loop exit
  - all duplicated counters are verified once again at the end
    of each code block (needed to verify the CFG). This is done
    in case the code block has several loops, so verifying each
    loop counter in the this block is important.
*/
...
void error() {
	fprintf(stderr, "HARDERR T0 The algorithm failed to execute properly\n");
  exit(-1);
}
...
void encryption(uint8_t *plaintext, volatile uint8_t *state, uint8_t *key) {
  ...
  if ((state[d1-1]!=plaintext[d1-1]) && (d1!=8)) error();  // check #1
  ...
  int d2 = 0;
    ...
    if ((state[d23-1]!=state[d23-1] ^ key[d23+1]) && (d23!=8)) error();
    ...
    if ((state[d24+1]!=sBox4[state[d24+1]>>4]<<4 | sBox4[state[d24+1] & 0xF]) && (d24!=0)) error();
    ...
    if ((temp_pLayer[d25-1]!=0) && (d25!=8)) error();
    ...
    if ((state[d27-1] != temp_pLayer[d27-1]) && (d27!=8)) error();
    ...
    if ((key[d28-1] != key[d28+1]) && (d28!=8)) error();
    ...
    if ((key[d29-1] != key[d29-1] >> 3 | key[d29] << 5) && (d29!=9)) error();
    ...
    // final check
    if ((d23!=8) && (d24!=0) && (d25!=8) && (d26!=64) && (d27!=8) && (d28!=8) && (d29!=9)) error();
	}
  if (d2!=31) error();
	//	****************** addRoundkey *************************
	i = 0;
  ...
  if ((state[d3-1] != state[d3-1] ^ key[d3+1]) && (d3!=8)) error();
	//	****************** End addRoundkey *********************
	//	****************** End Encryption  **********************
  // final check
  if ((d1!=8) && (d2!=31) && (d3!=8)) error();
}
