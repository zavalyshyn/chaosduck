...
void encryption(uint8_t *plaintext, volatile uint8_t *state, uint8_t *key) {
  ...
  int d1 = 0;
	//	****************** Set up state **************************
  for(i=0,d1=0;(i<8)&&(d1<8);i++,d1++) {
      state[i] = plaintext[i];
  }
  if (d1!=8) error(90);  // check #1
  if (i!=8) error(91);  // check #1

	i = 0;
  int d2 = 0;
	//	****************** Encryption **************************
	round=0;
	while((round<31)&&(d2<31)) {
		//	****************** addRoundkey *************************
		i=0;
    int d23 = 0;
		while((i<=7)&&(d23<=7)) {
    ...
    if (i!=0) error(118);
		...
		//	****************** pLayer ******************************
    int d25 = 0;
		for(i=0,d25=0;(i<8)&&(d25<8);i++,d25++) {
			temp_pLayer[i] = 0;
		}

    if ((temp_pLayer[d25-1]!=0) || (d25!=8)) error(127);
    if ((temp_pLayer[i-1]!=0) || (i!=8)) error(128);

    int d26 = 0;
		for(i=0,d26=0;(i<64)&&(d26<64);i++,d26++)	{
    ...
    // final check
    if ((d23!=8) || (d24!=0) || (d25!=8) || (d26!=64) || (d27!=8) || (d28!=8) || (d29!=9)) error(194);
	}
  if (d2!=31) error(196);
	//	****************** addRoundkey *************************
	i = 0;
  int d3 = 0;
	while((i<=7)&&(d3<=7)) {		//final key XOR
		state[i] = state[i] ^ key[i+2];
		i++;
    d3++;
	}
