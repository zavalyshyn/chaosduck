...
Hardening technique: function duplication (T2)
Details:
  - sensitive functions are duplicated and their results are compared
    before outputing final results
...
void error() {
	fprintf(stderr, "HARDERR T2 The algorithm failed to execute properly\n");
  exit(-1);
}
...
void encryption_check(uint8_t *plaintext, volatile uint8_t *state_check, uint8_t *key_check) {
	const uint8_t sBox4[] =	{0xc,0x5,0x6,0xb,0x9,0x0,0xa,0xd,0x3,0xe,0xf,0x8,0x4,0x7,0x1,0x2};
	//	Counter
	uint8_t i = 0;
	//	pLayer variables
  ...
  for(i=0;i<8;i++) {
	...
	while(round<31) {
		...
		i=0;
		while(i<=7) {
			state_check[i] = state_check[i] ^ key_check[i+2];
			i++;
		}
		...
		while(i>0) {
			i--;
			state_check[i] = sBox4[state_check[i]>>4]<<4 | sBox4[state_check[i] & 0xF];
		}
		...
		key_check[9] = save2;
		i = 0;
		save1 = key_check[0] & 7;	//61-bit left shift
		while(i<9) {
		...
		key_check[9] = sBox4[key_check[9]>>4]<<4 | (key_check[9] & 0xF);	//S-Box application
...
int main(int argc, char *argv[]){
	...
	uint8_t key_check[10];	// a key copy needed for a duplicated encryption func.
  ...
  encryption_check(plaintext, state_check, key_check);

  // cipher check for faults
  for (int d=0; d<8; d++) {
    if (state[d]!=state_check[d]) {
      error();
    }
  }
    ...
    if (state[i]!=state_check[i]) error();
